...
function Algorithm() {
  const data = useLoaderData({ from: '/algorithms/$key' });
  const params = useParams({ from: '/algorithms/$key' });
  useTextFragment();
  return (
    <div className="flex-1 p-4 pt-0 overflow-y-auto self-center w-full max-w-250">
      <AddToBookmark
        link={`/algorithms/${params.key}`}
...
export const Route = createFileRoute('/algorithms/$key')({
  component: Algorithm,
  loader: async ({ params }) => {
    return (
      (await import(`../../content/algorithms/${params.key}.md?raw`)) as {
        default: string;
      }
    ).default;
  },
